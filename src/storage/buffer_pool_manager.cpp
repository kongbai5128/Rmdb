...
void BufferPoolManager::update_page(Page *page, PageId new_page_id, frame_id_t new_frame_id) {
    ...
    if(!page_table_.count(page->get_page_id())) {
        page_table_.erase(page->get_page_id());
    }
    page->pin_count_ = 0;
    page->id_ = new_page_id;
    page_table_[new_page_id] = new_frame_id;  // 更新页表，不用清除旧的page->data_，因为它会被新页覆盖
}
...
Page* BufferPoolManager::fetch_page(PageId page_id) {
    ...
    page->is_dirty_ = false;  // 新读取的页面默认不是脏页
    page->id_ = page_id;  // 更新page的id
    page->pin_count_ = 1;  // 新页固定，pin_count置为1

    page_table_[page_id] = frame_id;  // 更新页表
    replacer_->pin(frame_id);  // 将frame_id加入replacer
    return page;  // 返回获取的页
}
/**
...
bool BufferPoolManager::unpin_page(PageId page_id, bool is_dirty) {
    ...
    if (page->pin_count_ <= 0) {
        return false;  // pin_count已经为0，不能取消固定
    }
    page->pin_count_--;
    if (page->pin_count_ == 0) {
        replacer_->unpin(frame_id);  // 将frame_id从replacer中移除
    }
    page->is_dirty_ = is_dirty | page->is_dirty_;  // 如果is_dirty为true，则将page标记为脏页
    return true;
}
...
bool BufferPoolManager::flush_page(PageId page_id) {
    ...
    page->is_dirty_ = false;  // 写回磁盘后，标记为非脏页
    return true;
}
...
Page* BufferPoolManager::new_page(PageId* page_id) {
    ...
    pages_[frame_id].pin_count_ = 1;  // 新页固定，pin_count置为1
    replacer_->pin(frame_id);  // 将frame_id加入replacer
...
bool BufferPoolManager::delete_page(PageId page_id) {
    ...
    Page* page = &pages_[frame_id];
    if (page->pin_count_ > 0) {
        return false;  // 目标页正在被使用，不能删除
    }
    if (page->is_dirty_) {//即使删除，事务提交的修改必须持久化
