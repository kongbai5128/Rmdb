...
void LRUReplacer::unpin(frame_id_t frame_id) {
    ...
    //  选择一个frame取消固定
    std::scoped_lock lock{latch_};
    auto frame_it = LRUhash_.find(frame_id);
    if (frame_it != LRUhash_.end()) {
        pin(frame_id);
    }
    LRUlist_.push_front(frame_id);  // 将frame_id添加到LRUlist的前面
    LRUhash_[frame_id] = LRUlist_.begin();  // 更新LRUhash，记录frame_id在LRUlist中的位置
}
